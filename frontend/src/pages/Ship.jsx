...
export default function Ship() {
  ...
  const [reqDeliveryDate, setReqDeliveryDate] = useState(
    getMinDeliveryDate(tomorrowStr),
  );

  // 2. Service type state
  const [mode, setMode] = useState(
    params.get("service")?.toUpperCase() || "OCEAN",
  );
  ...
  const originCandidates = useMemo(
    () => resolveGateway(originSearch, mode),
    [originSearch, mode],
  );
  const destCandidates = useMemo(
    () => resolveGateway(destSearch, mode),
    [destSearch, mode],
  );
  ...
  const removeCargo = (id) => {
    if (cargo.length <= 1) {
      toast("At least one cargo item is required");
      return;
    }
  ...
  const handleSubmitQuote = async () => {
    ...
    const quoteRecord = {
      ...
      details: {
        ...
        transitBreakdown: [
          {
            label: `Pickup leg (${pickupAddress ? "Door pickup" : "Road transit"})`,
            val: `${estimate.transitBreakdown.pickupDays} d`,
          },
          {
            label: `Origin dwell — ${loadType}`,
            val: `${estimate.transitBreakdown.originDwell} d`,
          },
          {
            label: `Main leg — ${estimate.mainDistanceNm} ${estimate.distanceLabel.toLowerCase()}`,
            val: `${estimate.transitBreakdown.linehaulDays} d`,
          },
          {
            label: "Schedule wait",
            val: `${estimate.transitBreakdown.scheduleWait} d`,
          },
          {
            label: "Destination dwell",
            val: `${estimate.transitBreakdown.destDwell} d`,
          },
          {
            label: "Delivery leg",
            val: `${estimate.transitBreakdown.deliveryDays} d`,
          },
        ],
      },
    };
    ...
    const shipmentRecord = {
      ...
      steps: [
        {
          label: "Booked",
          loc: `${originGw.city}, ${originGw.countryCode}`,
          ts: "Just now",
          done: true,
          current: true,
        },
        ...
        },
        {
          ...
          loc: `${destGw.city}, ${destGw.countryCode}`,
          ts: "Pending",
          done: false,
        },
        {
          ...
          <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-[1fr_360px]">
              ...
              <FormSection num={1} title="Route">
                  ...
                  <div className="relative">
                    ...
                    {originGw ? (
                      <div className="flex items-center gap-2 rounded-[10px] border-[1.5px] border-brand-marine bg-brand-marinePale px-3.5 py-2.5">
                        <span className="font-mono text-xs font-bold text-brand-marine">
                          {originGw.code}
                        </span>
                        <span className="flex-1 truncate text-[13px] font-medium text-brand-navy">
                          {originGw.name}
                        </span>
                        <button
                          type="button"
                        ...
                        {showOriginDropdown && (
                          <div className="absolute z-30 top-full left-0 right-0 mt-1 max-h-52 overflow-y-auto rounded-md2 border border-brand-line bg-white shadow-md2">
                            {originCandidates.length === 0 ? (
                              <div className="p-3 text-xs text-brand-slate">
                                No matching ports/airports found
                              </div>
                            ) : (
                              originCandidates.map((g) => (
                                ...
                                >
                                  <span className="font-mono font-semibold text-brand-marine">
                                    {g.code}
                                  </span>{" "}
                                  — {g.name} ({g.country})
                                </div>
                ...
                <div className="mt-[18px] grid grid-cols-1 gap-[18px] sm:grid-cols-2">
                  <div>
                    <label className="mb-2 block text-[13px] font-semibold text-brand-navy">
                      Pickup address{" "}
                      <span className="text-brand-slateLight font-normal">
                        (door pickup)
                      </span>
                    </label>
                    <input
              ...
              <FormSection num={3} title="Shipment details">
                <div className="space-y-4">
                  {cargo.map((item, index) => (
                    <div
                      key={item.id}
                      className="rounded-md2 border border-brand-line bg-white p-5 shadow-sm2"
                    >
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-4">
                          <div>
                            ...
                            <input
                              ...
                              value={item.quantity}
                              onChange={(e) =>
                                updateCargo(item.id, "quantity", e.target.value)
                              }
                              placeholder="1"
                              className={brandInputStyle}
                            />
                      ...
                      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={item.hs_code}
                            onChange={(e) =>
                              updateCargo(item.id, "hs_code", e.target.value)
                            }
                  ...
                  {hasCustomerCode && (
                    <div className="mt-3 max-w-sm">
                      <input
                        ...
                        onChange={(e) =>
                          setExistingCustomerCode(e.target.value)
                        }
            ...
            <div className="sticky top-[92px]">
              <div className="rounded-lg2 bg-brand-navy p-7 text-white shadow-md2">
                ...
                <div className="rounded-md2 bg-white/5 p-4 text-xs leading-relaxed text-slate-300">
                  Fill in every section of the form, then click below. Your
                  quotation will be generated after a short processing delay —
                  nothing is calculated or shown until you submit.
                </div>
                ...
                <div className="mt-4 text-[11px] leading-relaxed text-slate-400">
                  Final rates confirmed by your account manager. Estimate
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-brand-line text-center">
            ...
            <p className="mt-2 text-sm text-brand-slate leading-relaxed">
              Please log in or create an account to select route origins and
              generate live freight quotations.
            </p>
...
const brandInputStyle =
  "w-full rounded-[10px] border-[1.5px] border-brand-line bg-white py-3 px-3.5 font-sans text-[14px] text-brand-navy transition-colors focus:border-brand-marine focus:outline-none placeholder:text-brand-slateLight/60";
