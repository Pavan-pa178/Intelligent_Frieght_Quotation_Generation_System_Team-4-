...
import { Lock, Mail, Phone, Calendar, Plus, Search, Package, ArrowRight, Ship, Plane, Truck } from 'lucide-react'
import PageBanner from '../components/PageBanner'
import StatusBadge from '../components/StatusBadge'
import { useApp } from '../context/AppContext'
import { useToast } from '../context/ToastContext'

const SERVICE_ICON = { Ocean: Ship, Air: Plane }
            ...
            <div className="grid grid-cols-1 items-start gap-7 lg:grid-cols-[340px_1fr]">
              ...
              <div className="sticky top-[92px] rounded-lg2 border border-brand-line bg-white p-[30px]">
                <div className="mb-[22px] border-b border-brand-line pb-6 text-center">
                  ...
                  <h3 className="mb-1 text-lg">{user.name}</h3>
                  <p className="text-[13px] text-brand-slate">{user.company}</p>
                </div>
                <div className="mb-[22px] space-y-3">
                  <MetaRow icon={Mail} text={user.email} />
                  <MetaRow icon={Phone} text={user.phone} />
                  <MetaRow icon={Calendar} text={`Member since ${user.since}`} />
                </div>
                <div className="mb-[22px] grid grid-cols-2 gap-2.5">
                  <StatBox n={shipments.length} label="Shipments" />
                  <StatBox n={shipments.filter((s) => s.status !== 'Delivered').length} label="Active" />
                </div>
                <button className="mb-2.5 w-full rounded-lg border-[1.5px] border-brand-line bg-white py-2.5 text-[13.5px] font-semibold shadow-sm2">Edit profile</button>
                    ...
                    {shipments.map((s) => (
                      ...
                      >
                        <div className="flex h-[42px] w-[42px] flex-shrink-0 items-center justify-center rounded-[10px] bg-brand-marinePale text-brand-marine">
                          {s.service.includes('Ocean') ? <Ship className="h-5 w-5" /> : s.service.includes('Air') ? <Plane className="h-5 w-5" /> : <Truck className="h-5 w-5" />}
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 text-[14.5px] font-semibold">
                            {s.from} <ArrowRight className="h-3.5 w-3.5 text-brand-slateLight" /> {s.to}
                          </div>
                          <div className="mt-1 font-mono text-xs text-brand-slateLight">{s.tn} · {s.date}</div>
                        </div>
                        <div className="ml-14 flex-shrink-0 text-right sm:ml-0">
                          <StatusBadge status={s.status} />
                          <div className="mt-1 font-mono text-[14.5px] font-semibold">₹{s.cost.toLocaleString('en-IN')}</div>
                        </div>
                      </button>
                    ))}
                  </div>
